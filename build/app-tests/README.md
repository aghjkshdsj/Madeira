...
The library scans `Documents/wine/drive_c/Games` for EXEs. Installer/crash helper names and symbolic links are excluded. Import Game Folder copies the complete folder using coordinated security-scoped access. Custom EXE copies only the executable; adjacent game data must be supplied separately. Games are never downloaded or bundled.
